...
class Generator {
  ...
  constructor(
    ...
    const invalidOptions = getInvalidOptions(
      GENERATOR_OPTIONS,
      ...
      Object.defineProperty(this.templateParams, key, {
        ...
        get() {
          if (
            ...
            !self.templateConfig.parameters[key]
          ) {
            throw new Error(
  ...
  async generate(asyncapiDocument) {
    if (!(asyncapiDocument instanceof AsyncAPIDocument))
      throw new Error(
        'Parameter "asyncapiDocument" must be an AsyncAPIDocument object.'
      );
  ...
  async generateFromString(asyncapiString, parserOptions = {}) {
    if (!asyncapiString || typeof asyncapiString !== "string")
  ...
  async generateFromURL(asyncapiURL) {
    ...
    if (this.mapBaseUrlToFolder.url) {
      parserOptions.resolve = {
        resolver: getMapBaseUrlToFolderResolver(this.mapBaseUrlToFolder),
      };
    }
  ...
  generateDirectoryStructure(asyncapiDocument) {
    ...
    asyncapiDocument.allSchemas().forEach((schema, schemaId) => {
      if (schema.type() === "object") objectMap[schemaId] = schema;
    });
